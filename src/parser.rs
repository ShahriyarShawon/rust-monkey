...
impl Parser {
    pub fn new(l: Lexer) -> Parser {
        let mut p = Parser {
            l,
            cur_token: Token { token_type: TokenType::ILLEGAL, literal: "\0".to_string() },
            peek_token: Token { token_type: TokenType::ILLEGAL, literal: "\0".to_string() },
            errors: Vec::new()
        };
    ...
    pub fn peek_error(&mut self, token: &TokenType) {
        let msg = format!("expected next token to be {:?}, got {:?} instead", token, self.peek_token.token_type);
...
mod tests {
    use crate::lexer;
    use super::{LetStatement, Node, Parser, Statement};
...
"#;
        ...
        if program.statements.len() != 3 {
            println!("program statements does not container 3 statements, got {}", 
    ...
    fn test_let_statement(stmt: &Box<dyn Statement> , name: &str) -> bool {
        ...
        let letstmt = match stmt.as_any().downcast_ref::<LetStatement>() {
            ...
            None =>  {
                print!("statement is not let statement {:?}", stmt.statement_type());
                return false;
            }
        };
    
        if letstmt.name.value != name {
            println!("let statement value not {}, instead got {}", name, letstmt.name.value);
            return false;
        }

        if letstmt.name.token_literal() != name {
            println!("letstatement token literal not {}, instead got {}", name, letstmt.name.token_literal());
            return false;
        }
        true
    }
